...
public class RetentionManager {
    ...
    public RetentionManager(Context context, HttpLogger.Period retentionPeriod) {
        this.context = context;
        period = toMillis(retentionPeriod);
        prefs = context.getSharedPreferences(PREFS_NAME, 0);
        cleanupFrequency = (retentionPeriod == HttpLogger.Period.ONE_HOUR) ?
    ...
    private long toMillis(HttpLogger.Period period) {
        switch (period) {
            case ONE_HOUR:
