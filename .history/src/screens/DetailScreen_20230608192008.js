...
const CardDetail = ({
  ...
  const handleAddToCart = async () => {
    try {
      ...
      const response = await api.post('/accessories/addToCart', { product });
  ...
  In this example, we import the AsyncStorage module and the api Axios instance from separate files. The access token is retrieved from AsyncStorage using AsyncStorage.getItem('user'), and if it exists, it is set in the Authorization header of the api instance.
  
  Inside the handleAddToCart function, we use an async/await syntax to make the API request using api.post. We provide the endpoint /accessories/addToCart and the product object as the request payload.
  
  The API response is captured in the response variable, and you can handle it as needed. In this example, we log the response.data to the console.
  
  Remember to replace the './api' import with the correct path to the file where you define your api Axios instance.
  
  Note: Make sure you have properly configured AsyncStorage and have stored the access token in the 'user' key before using this code.
