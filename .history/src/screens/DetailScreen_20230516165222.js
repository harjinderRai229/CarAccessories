...
const CardDetail = ({ route }) => {
  const { productId , product} = route.params; // Get the product ID from the route parameters
  ...
  if (!products) {
    return (
      <View style={styles.errorContainer}>
        <Text>Error loading products details</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Image style={styles.image} source={{ uri: products.imageUrl[0] }} />
      <Text style={styles.title}>{products.prodName}</Text>
